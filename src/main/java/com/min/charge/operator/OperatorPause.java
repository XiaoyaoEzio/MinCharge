...
public class OperatorPause {

	private static final Logger logger = Logger.getLogger(OperatorPause.class);
	
	public JsonResult pause(Client client,String deviceSn, String path){
		JsonResult result = new JsonResult();
		SqlSession session = MybaitsConfig.getCurrent();
		String jsonString = ChargeApi.operator(OperatorTypeEnum.Pause.getCommand(), deviceSn, path);
		String method = "";
		try {
			...
			if (method != null || !"".equals(method)) {
				...
				if (status != 1) {
					...
					return JsonResult.code(ErrorCodeEnum.COMMAND_PAUSE_FAILD);
				}
				bufferRecord.setLastPauseTime(new Date());
				bufferRecord.setOrderStatusEnum(OrderStatusEnum.Pause);
