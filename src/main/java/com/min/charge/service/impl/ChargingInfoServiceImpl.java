...
public class ChargingInfoServiceImpl implements ChargingInfoService {
    ...
    public JsonResult refresh(String token, String deviceSn) {
        ...
        chargeInfo.chargeTime = (int) ((now.getTime() - record.getStartTime().getTime() - record
                .getTotalPauseTime()) / 1000);
        ...
        if (record.getOrderStatusEnum().compareTo(OrderStatusEnum.Pause) == 0) {
            chargeInfo.chargeTime = (int) ((record.getLastPauseTime().getTime() - record.getStartTime().getTime()) /
                    (60 * 1000)) - record.getTotalPauseTime() / (60 * 1000);
            chargeInfo.totalPauseTime = (int) ((now.getTime() - record.getLastPauseTime().getTime()) / (60 * 1000)) +
					record.getTotalPauseTime() / (60 * 1000);
        }
